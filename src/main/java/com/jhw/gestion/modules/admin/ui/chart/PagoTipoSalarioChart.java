 ...
 */
public class PagoTipoSalarioChart extends _MaterialBarChart implements Update {

    public PagoTipoSalarioChart() {
    ...
    public void update() {
        removeAllBars();
        try {
            HashMap<TipoSalarioDomain, BigDecimal> h = NominaSwingModule.pagoSalarioUC.reporteSalarioPorTipo();
            for (Map.Entry<TipoSalarioDomain, BigDecimal> entry : h.entrySet()) {
                addBar(entry.getValue(), 0, entry.getKey().getNombreTipoSalario());
            }
            this.getChart().setTitle("Salarios según tipos (Conversión a: " + MonedaHandler.getMonedaBase() + ")");
        } catch (Exception ex) {
